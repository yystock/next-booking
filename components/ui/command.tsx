  ...
  ({ className, ...props }, ref) => (
    <div className="flex items-center border-b px-3" cmdk-input-wrapper="">
      ...
      <CommandPrimitive.Input
        ref={ref}
        className={cn(
          "flex h-11 w-full rounded-md bg-transparent text-sm outline-none placeholder:text-slate-100/90 disabled:cursor-not-allowed disabled:opacity-50",
          className
        )}
        {...props}
      />
...
const CommandList = React.forwardRef<React.ElementRef<typeof CommandPrimitive.List>, React.ComponentPropsWithoutRef<typeof CommandPrimitive.List>>(
  ({ className, ...props }, ref) => (
    <CommandPrimitive.List ref={ref} className={cn("bg-transparent max-h-[300px] overflow-y-auto overflow-x-hidden", className)} {...props} />
  )
);
...
const CommandItem = React.forwardRef<React.ElementRef<typeof CommandPrimitive.Item>, React.ComponentPropsWithoutRef<typeof CommandPrimitive.Item>>(
  ({ className, ...props }, ref) => (
    <CommandPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex my-2 cursor-pointer select-none items-center rounded-xl px-3 py-2 text-sm outline-none aria-selected:bg-accent-foreground aria-selected:text-accent data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        className
      )}
      {...props}
    />
